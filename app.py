...
# Tabs layout
tabs = st.tabs(["📈 Predict", "📊 Model Performance", "📋 Logs", "📉 Visualizations", "ℹ️ About"])
...
with tabs[0]:
    ...
    if st.button("Generate Prediction"):
        ...
        # Download result
        st.download_button("⬇️ Download Prediction", data=pd.DataFrame([log_entry]).to_csv(index=False), file_name="prediction.csv")
...
with tabs[2]:
    ...
    if os.path.exists("prediction_log.csv"):
        ...
        st.download_button("⬇️ Download Full Log", data=log_data.to_csv(index=False), file_name="prediction_log.csv", mime="text/csv")
    else:
